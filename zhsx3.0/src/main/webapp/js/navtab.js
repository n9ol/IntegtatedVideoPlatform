layui.define(['element'], function(exports){
   var  element = layui.element(),
        ...
        LarryTab.prototype.tabAdd = function(data){
		    ...
		    if(tabIndex === -1){
		    	globalTabIdIndex++;
		    	var content = '<iframe src="' + data.href + '" lay-id="' + globalTabIdIndex + '" class="larry-iframe"></iframe>';
			    var title = '';
			    // 若icon有定义
			    ...
			    if(_this.config.closed) {
				    title += '<i class="layui-icon layui-unselect layui-tab-close" lay-id="' + globalTabIdIndex + '">&#x1006;</i>';
			    }
			    //添加tab
			    ...
			    //iframe 自适应
			    ELEM.contentBox.find('iframe[lay-id=' + globalTabIdIndex + ']').each(function() {
			    	$(this).height(ELEM.contentBox.height());
			    });
			    if(_this.config.closed) {
				//监听关闭事件
				    ELEM.titleBox.find('li').children('i.layui-tab-close[lay-id=' + globalTabIdIndex + ']').on('click', function() {
				    	element.tabDelete(ELEM.tabFilter, $(this).attr("lay-id")).init();
				    });
			    };
			    //切换到当前打开的选项卡
				...
				//自动刷新
				if (_this.config.autoRefresh) {
					_this.config.elem.find('div.layui-tab-content > div').eq(tabIndex).children('iframe')[0].contentWindow.location.reload();
				}
		    }
        };
