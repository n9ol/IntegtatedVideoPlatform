...
WebUploader.Uploader.register({
	...
	beforeSendFile:function(file){
		...
		.then(function(val){
			fileMd5 = val;
			$.getJSON(ctx+"/adminOffLine/verifyVideoRes", { fileMd5:fileMd5,size:file.size }, function(data){
			   if(data){
		...
		if(title != "文件已存在"){
			 ...
			 $.ajax({
			   ...
			   success: function(msg){
				   addData(msg,file.name,fileMd5,file.size);
				   deferred.resolve();
			   }
			});
		   return deferred.promise();
...
//添加文件到数据库
function addData(msg,name,fileMd5,bak2){
	$.getJSON(ctx+"/adminOffLine/insterVideoRes", {title:name,videoPath:msg.filePath,bak1:fileMd5,bak2:bak2}, function(json){
	   if(json.id != ""){
		   videoTranscoding(json.id,msg.filePath,json.webpath);
		   getVideoResData();
	   }
	});
}
...
uploader.on( 'filesQueued', function(files) {
	for (var int = 0; int < files.length; int++) {
		var file = files[int];
		 $.ajax({
		   type: "POST",
		   url: ctx+"/adminOffLine/fileUpload",
